import {Marker} from 'src:Marker.js'; // Nick:
...
export class MyMarker extends Marker { // Nick:
	...
	_initIcon() {
		...
		const icon = options.icon.createIcon(this._icon, options.keyboard); // Nick:
		let addIcon = false;
		...
		if (icon !== this._icon) {
			...
			if (icon.tagName === 'IMG' || icon.tagName === 'INPUT') { // Nick;
