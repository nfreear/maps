...
export class Icon extends Class {
	...
	createIcon(oldIcon, hasKeyboard) { // Nick:
	...
	_createIcon(name, oldIcon, hasKeyboard) { // Nick:
		...
		const img = this._createImg(src, oldIcon && oldIcon.tagName === 'IMG' ? oldIcon : null, hasKeyboard); // Nick:
	...
	_createImg(src, el, hasKeyboard) { // Nick:
		el ??= document.createElement(hasKeyboard ? 'input' : 'img'); console.debug('>>>_createImg');
    if (hasKeyboard) {
      el.type = 'image';
    }
		el.src = src;
